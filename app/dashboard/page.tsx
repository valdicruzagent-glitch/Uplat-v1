...
const ALLOWED_EMAILS = ['rafael@example.com', 'belkis@example.com']; // TODO: move to env
  ...
  useEffect(() => {
    const checkAndFetch = async () => {
      ...
      const [
        ...
        supabase.from('agent_reviews').select('rating'), // for overall average
        ...
        supabase.from('listings').select('id, title, (listing_inquiries: listing_id)').order('created_at', { ascending: false }).limit(10), // we'll count inquiries per listing later
        supabase.from('listings').select('country').eq('status', 'published'),
      ]);

      // Compute avgAgentRating from raw ratings
      const ratings = agentReviewsForAvg.data?.map(r => r.rating as number) || [];
      const avgAgentRating = ratings.length ? ratings.reduce((a, b) => a + b, 0) / ratings.length : 0;

      // Compute top listings by inquiry count (since we didn't get counts directly)
      ...
      // Compute top countries by published listings
