...
export default function HomeEn() {
  ...
  const handleSell = () => {
    window.location.href = '/en/signin?next=/sell';
  };
      ...
      <header className="sticky top-0 z-[1001] border-b border-zinc-200 bg-white/80 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/80">
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4">
          ...
          {user ? (
            ...
            <Link href="/en/signin" className="md:hidden px-4 py-2 text-sm font-medium rounded bg-blue-600 text-white hover:bg-blue-700">{t.signInTitle}</Link>
          )}
      ...
      {mobileOpen && (
        <div className="fixed inset-x-0 top-14 bottom-0 z-40 bg-zinc-950/95 p-6 md:hidden overflow-y-auto">
          ...
          <div className="mt-16 flex flex-col gap-6">
            ...
            <Link href="/en" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 text-lg font-medium text-zinc-50 transition-colors hover:text-zinc-300">
              <span>→</span> {t.buy}
            </Link>
            <Link href="/en" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 text-lg font-medium text-zinc-50 transition-colors hover:text-zinc-300">
              <span>→</span> {t.rent}
